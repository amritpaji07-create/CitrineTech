document.addEventListener("DOMContentLoaded", function () {
    ...
    new Swiper(".patroneSwiper", {
        ...
        direction: "horizontal", // only "horizontal" or "vertical" allowed
    ...
    const counterObserver = new IntersectionObserver(entries => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                animateCounters();
                counterObserver.disconnect(); // remove if you want repeat animation
            }
        });
    }, { threshold: 0.5 });
    ...
    if (heroText) {
        const sentences = [
            "Crafting Digital Futures with Precision.",
            "Empowering Innovation Through Technology.",
            "Design. Develop. Deliver — Seamlessly.",
            "Transforming Ideas into Impactful Solutions.",
            "Your Vision, Our Code — Perfectly Aligned."
        ];
        let index = 0, charIndex = 0, isDeleting = false;
        const typingSpeed = 80, deletingSpeed = 50, delayBetween = 1500;
        function typeEffect() {
            const current = sentences[index];
            heroText.textContent = isDeleting
                ? current.substring(0, charIndex--)
                : current.substring(0, charIndex++);
            heroText.classList.add("typewriter-glow");
    ...
    if (toggleBtn) {
        toggleBtn.addEventListener('click', () => {
            ...
            toggleBtn.textContent = isExpanded ? 'Show Less 🔽' : 'More Projects ⚡';
        });
    }
            ...
            Array.from(this.files).forEach(file => {
                if (file.type.startsWith("image/")) {
                    ...
                    reader.onload = e => {
                        ...
                        img.className = "w-32 h-32 object-cover rounded-md border border-gray-300";
    ...
    if (videoInput && videoPreviewContainer) {
        videoInput.addEventListener('change', function (event) {
            ...
            if (file && file.type.startsWith('video/')) {
                ...
                videoPreviewContainer.innerHTML = '<p class="text-red-600">Please select a valid video file.</p>';
            }
        });
    }
});
